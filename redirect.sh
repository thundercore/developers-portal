...
do
  ...
  aws s3 cp $filename.html s3://$url/docs/$path --website-redirect "$redirect_url/docs/$redirect_path"
...
aws s3 cp $filename.html s3://$redirect_url/docs/$redirect_path --website-redirect "$url/docs/$path"
